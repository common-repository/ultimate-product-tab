<?php
/*
Plugin Name: Ultimate Product Tab
Plugin URI: http://webeeoo.com/wp_plugin/product/sample-product/
Description: This plugin will enable an awesome wc ultimate custom product tab. 
Author: Md. Shiddikur Rahman
Author URI: http://phpdev.us/siddik
Version: 1.0
*/
/**
*Ultimate Product Tabs
*/
class Ultimate_Custom_Product_Tabs{
    /**
     * $post_type
    ...
    */
    ...
    function woocommerce_settings_tabs_array( $settings_tabs ) {
        $settings_tabs[$this->id] = __('UWP Custom Tabs','UWP');
    ...
    function get_settings(){
        $settings = array(
            'section_title' => array(
                'name'     => __('UWP Custom Tabs','UWP'),
                'type'     => 'title',
                'desc'     => '',
                'id'       => 'wc_'.$this->id.'_section_title'
            ),
            'title' => array(
                'name'     => __( 'Global Custom Tabs', 'UWP' ),
                'type'     => $this->post_type,
                'desc'     => __( 'Start typing the Custom Tab name, Used for including custom tabs on all products.', 'UWP' ),
                'desc_tip' => true,
                'default'  => '',
        ...
        ?><tr valign="top">
            ...
            <td class="forminp forminp-<?php echo sanitize_title( $field['type'] ) ?>">
                <p class="form-field custom_product_tabs">
                    <select id="custom_product_tabs" style="width: 50%;" name="<?php echo $field['id'];?>[]" class="ajax_chosen_select_tabs" multiple="multiple" data-placeholder="<?php _e( 'Search for a custom tab&hellip;', 'UWP' ); ?>">
                        <?php   
    ...
    function woocommerce_product_write_panels() {
        ...
        $fields = array(
            array(
                'key'   => 'custom_tabs_ids',
                'label' => __( 'Select Custom Tabs', 'UWP' ),
                'desc'  => __( 'Start typing the Custom Tab name, Used for including custom tabs.', 'UWP' )
            ),
            array(
                'key'   => 'exclude_custom_tabs_ids',
                'label' => __( 'Select Global Tabs to exclude', 'UWP' ),
                'desc'  => __( 'Start typing the Custom Tab name. used for excluding global tabs.', 'UWP' )
            )
        );
        ?>
            ...
            foreach ($fields as $f) {
                ...
                <div class="options_group">
                    <p class="form-field custom_product_tabs">
                        ...
                        <select style="width: 50%;" id="<?php echo $f['key']; ?>" name="<?php echo $f['key']; ?>[]" class="ajax_chosen_select_tabs" multiple="multiple" data-placeholder="<?php _e( 'Search for a custom tab&hellip;', 'UWP' ); ?>">
                            <?php                           
                                foreach ( $this->get_custom_tabs_list() as $id => $label ) {
                                    $selected = in_array($id, $_ids)?  'selected="selected"' : '';
                                    echo '<option value="' . esc_attr( $id ) . '"'.$selected.'>' . esc_html( $label ) . '</option>';
    ...
    function custom_product_tabs_post_type() {
        $labels = array(
            'name'                => _x( 'Product Tabs', 'Post Type General Name', 'UWP' ),
            'singular_name'       => _x( 'Product Tab', 'Post Type Singular Name', 'UWP' ),
            'menu_name'           => __( 'product Tabs', 'UWP' ),
            'parent_item_colon'   => __( '', 'UWP' ),
            'all_items'           => __( 'Product Tabs', 'UWP' ),
            'view_item'           => __( '', 'UWP' ),
            'add_new_item'        => __( 'Add Product Tab', 'UWP' ),
            'add_new'             => __( 'Add New', 'UWP' ),
            'edit_item'           => __( 'Edit Product Tab', 'UWP' ),
            'update_item'         => __( 'Update Product Tab', 'UWP' ),
            'search_items'        => __( 'Search Product Tab', 'UWP' ),
            'not_found'           => __( 'Not found', 'UWP' ),
            'not_found_in_trash'  => __( 'Not found in Trash', 'UWP' ),
        );
        $args = array(
            'label'               => __( 'Product Tabs', 'UWP' ),
            'description'         => __( 'Custom Product Tabs', 'UWP' ),
            'labels'              => $labels,
...
}//end Ultimate_Custom_Product_Tabs class.
new Ultimate_Custom_Product_Tabs();